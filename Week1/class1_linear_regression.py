...
def linear_grad_func(theta, x, y):
    ...
    grad = np.divide(np.dot((linear_val_func(theta,x) - y).T,np.c_[np.ones(x.shape[0]), x]),x.shape[0])
    return grad
...
def linear_cost_func(theta, x, y):
    ...
    cost = np.divide(np.sum(np.square(linear_val_func(theta,x) - y),axis=0),2*y.shape[0])
    return cost
